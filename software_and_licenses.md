<!-- Software is computer programs, including web applications, desktop applications, operating system code, services needed for other software to function, programs needed for hardware to function, like drivers, etc.
While hardware contitutes the backbone of computing, letting software be built upon it that employs the hardware to provide or implement certain functions, bringing users a way of making use of the features offered by the hardware, or building up further abstraction for higher-level software to use, like an operating system would abstract away virtual memory, provide a mechanism for process creation and management, implement abstractions for hardware interrupts, like I/O hardware callbacks. 
Software may provide essential libraries for other software to use, like libc, Direct3D or OpenGL.
In the end, the software of the highest level are the programs computer users use every day, like the browser, chat apps, text editors, computer games, and a wide variety of utility programs, like photo or video viewers, book readers, image editing software, torrenting software, etc. -->
...
<!-- 
More and more of the today world's economy is invested into software and it has become truly important ([example source][1]: 2.5 million jobs, 17% R&D share, software develops twice the average salary of other groups, contributes to economic growth, etc.) -->
...
<!-- With hardware starting to be bought by customers came the idea of selling software to be installed on that hardware.
Initially hardware manufacturers [sold software bundled with the hardware][2], only allowing programs for their proprietary OS to be (pre-)installed on the machine [citation????][], thus establishing the monopoly over all software products that could be used by their customers.
Later [citation????][], OS's like Windows or MacOS would allow programmers to develop and sell software for their OS, while also selling their own proprietary software, e.g. MS's Office Pack. -->
...
<!-- At the same time, some people would distribute software for free, e.g. the Linux OS, GNU software, allowing people to also view their source code and contribute to it, which would get the name "open source".
This idea is pretty popular and common nowadays, with millions of developers eagerly distributing their work for free, letting people view, modify and even reuse their code via platforms like GitHub. -->
...
Prin dreptul de autor se presupune că orice operă (în acest caz program) creat de o persoană (autor) este proprietatea acestei persoane. 
...
They would still have some moral rights for their work, but legally the author would now be considered the employer. -->
...
<!-- Licenses can be categorized in terms of how many restrictions they impose: a proprietary license is at the first end of the scale, meaning no distributing of the software, no reusing or reselling, and the license is final, and permissive open source licenses at the other end of the scale, allowing code distribution, reuse, reselling, and sometimes even sublicensing. -->
...
<!-- Licenses may add rules defining what should happen when the project they are applied to is to be used within another project.
For example, certain licenses, like the GNU GPL-2 (General Public License) stipulates that the code be licensed under the same GNU GPL-2 license, thus forcing it to be part of open-source software.
This may prevent the use of GNU libraries in projects where authors want to keep their source closed or be able to sell their product.
Certain permissive licenses like the MIT license allow software reuse in derivative works, only forcing the included, either original or modified, sources of the dependency to contain the original copy of the license file. -->
...
<!-- Usually, by "license" people mean "license template", that is, a particular well-known named set of rules, that's already widely accepted in the community. 
Some examples: MIT Licence, GNU General Public License, Boost License.
An author would generally use one of these instead of devising their own, deciding which one to use based on the restrictions each one implies that the author wants their software to have. -->
...
<!-- Sometimes a project may depend on another project, which may contain contradictory statements (rules) in its license to the license under which it is already licensed.
This may force the author to **relicense** their project in order to be able to use the dependency, or to agree with the author of the dependency to allow them to use it under a different license. -->
...
<!-- There may be contradictory statements within the rules of the dependencies, making it impossible to use them in combination with each other, which may also force the author to relicense their product.
Another solution in this case may be **multi-licensing**, where the software would be distributed under more than a single license at once. 
For example, Mozilla, a free software organization, tri-licensed the source code of their Mozilla Application Suite, under Mozilla Public License (MPL) 1.1, GNU General Public License (GPL) 2.0 or GNU Lesser General Public License (LGPL) 2.1 [source][multimozilla], before updating their MPL license to a GNU 2.0 compatible one. -->
...
<!-- The chosen rules are legally enforced by the license, so if the author has their code used for another project and sold, while their license does not allow it, they have a legal right to sue those who did it. -->
...
<!-- WRONG!!!! That's why when a programmer got hired, they would typically sign a contract that would override this rule.
The contract would contain a clause that would grant the authorship rights of the code written at work to the company that had hired the original author — the programmer.
Without such a contract, the company would have no legal rights to sell the product, because it technically didn't belong to it. -->

<!-- Today, it is important for software developers to be aware of licensing, as it may even get them into legal trouble when disregarded in one's derivative work, or even make them lose the profits of their product [examples?????][].
Also, due to the lack of license implying proprietary conditions to code reuse, if an author wants to share their software with the world, allowing others to build upon it, they should make the code open source and, most importantly, license it under a permissive license.
Some licenses force derivative works to keep the copyright notices, thus the identity of the initial author may get recognized by their users.
Even though rare, but if the author of a derivative work deems the initial author's foundation really useful and impactful, they may even get sponsored to encourage them to make more useful open source products.
E.g. Github includes an option to add a "Sponsor Me" button on author's profile or in a project's repository, to easily take donations from the users of their free software or from package consumers.

In practice, licensing software is done simply by adding a license file at the root of the repository that contains your code. -->
